...
// Get eligible users for new conversations
// As a team member, can only chat with admins, other members, or assigned applicants
$query = "SELECT u.id, u.first_name, u.last_name, u.profile_picture, u.user_type, 
          CASE WHEN u.user_type = 'applicant' THEN 'Client' ELSE u.user_type END AS role_display
          FROM users u
          WHERE (u.user_type IN ('admin') OR 
                (u.user_type = 'member' AND u.id != ?) OR
                (u.user_type = 'applicant' AND EXISTS (
                    SELECT 1 FROM applications a 
                    JOIN application_assignments aa ON a.id = aa.application_id 
                    WHERE a.user_id = u.id AND aa.team_member_id = ? AND aa.status = 'active'
                )))
          AND u.status = 'active' 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_direct_conversation'])) {
    ...
    // Validate recipient is eligible
    ...
    if ($valid_recipient) {
        ...
        if ($existing_conversation) {
            ...
            try {
                ...
                $stmt->execute();
                $recipient_type = $stmt->get_result()->fetch_assoc()['user_type'];
                $stmt->close();
                ...
                $stmt = $conn->prepare($query);
                $stmt->bind_param('iis', $new_conversation_id, $recipient_id, $recipient_type);
                $stmt->execute();
                $stmt->close();
        ...
        $error_message = "Invalid recipient selected";
    }
}
...
$query = "SELECT c.id, c.title, a.reference_number
          ...
          AND aa.team_member_id = ? AND aa.status = 'active'
          ORDER BY c.last_message_at DESC";
                ...
                <div class="conversations-list">
                    <?php if (empty($conversations)): ?>
                        <div class="empty-conversations">
                            ...
                            <p>Start a new conversation with an admin, team member or client</p>
                        </div>
                    <?php else: ?>
                                ...
                                <div class="conversation-avatar">
                                    ...
                                    <?php else: ?>
                                        <div class="initials">
                                            ...
                                            $query = "SELECT CONCAT(u.first_name, ' ', u.last_name) as name, u.user_type
                                                    FROM conversation_participants cp
                                                    JOIN users u ON cp.user_id = u.id
                                        ...
                                        <h4>
                                            ...
                                            if ($conversation['type'] === 'group') {
                                                ...
                                                if ($other_user['user_type'] === 'admin') {
                                                    ...
                                                    $badge_text = 'Admin';
                                                } elseif ($other_user['user_type'] === 'applicant') {
                                                    $badge_class = 'client-badge';
                                                    $badge_text = 'Client';
                                                } elseif ($other_user['user_type'] === 'member') {
                                                    $badge_class = 'member-badge';
                                                    $badge_text = 'Team';
                                                }
                                                
                                                if (!empty($badge_text)) {
                                                    echo ' <span class="user-badge ' . $badge_class . '">' . $badge_text . '</span>';
                                                }
                                            }
                                            ?>
                                        </h4>
        ...
        <div class="messages-area">
                    ...
                    <div class="conversation-info">
                        ...
                        <div>
                            ...
                            <p>
                                ...
                                <?php else: ?>
                                    <?php
                                    // Find the other participant to display their role
                                    foreach ($conversation_participants as $participant) {
                                        if ($participant['id'] != $user_id) {
                                            if ($participant['user_type'] === 'admin') {
                                                echo 'Administrator';
                                            } elseif ($participant['user_type'] === 'applicant') {
                                                echo 'Client';
            ...
            <?php else: ?>
                <div class="empty-chat-state">
                    ...
                    <p>Connect with team members, administrators, or assigned clients</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
            ...
            <div class="modal-body">
                <form action="messages.php" method="POST" id="newConversationForm">
                    <div class="form-group">
                        <label for="recipient_id">Select Contact</label>
                        <div class="participant-selection">
                            <?php if (empty($eligible_users)): ?>
                                <p class="no-contacts">No eligible contacts found. You can only message administrators, other team members, or clients assigned to your applications.</p>
                            <?php else: ?>
                                <?php 
                                ...
                                if (!empty($grouped_users['admin'])): 
                                ?>
                                    <div class="contact-group">
                                        <h4 class="contact-group-title">Administrators</h4>
                                        <?php foreach ($grouped_users['admin'] as $admin): ?>
                                            <div class="participant-option">
                                                <label>
                                                    <input type="radio" name="recipient_id" value="<?php echo $admin['id']; ?>" required>
                                                    <div class="participant-info">
                                                        <div class="participant-avatar small">
                                                            <?php if (!empty($admin['profile_picture'])): ?>
                                                                <img src="../../uploads/profiles/<?php echo $admin['profile_picture']; ?>" alt="Profile">
                                                            <?php else: ?>
                                                                <div class="initials">
                                                        ...
                                                        <div class="participant-details">
                                                            ...
                                                            <span class="participant-role admin-badge">
                                                                Administrator
                                                            </span>
                                                        </div>
                                                    </div>
                                ...
                                <!-- Display team members next -->
                                <?php if (!empty($grouped_users['member'])): ?>
                                    <div class="contact-group">
                                        <h4 class="contact-group-title">Team Members</h4>
                                        <?php foreach ($grouped_users['member'] as $member): ?>
                                            <div class="participant-option">
                                                <label>
                                                    <input type="radio" name="recipient_id" value="<?php echo $member['id']; ?>" required>
                                                    <div class="participant-info">
                                                        <div class="participant-avatar small">
                                                        ...
                                                        <div class="participant-details">
                                                            ...
                                                            <span class="participant-role member-badge">
                                                                Team Member
                                                            </span>
                                                        </div>
                                                    </div>
                                ...
                                <!-- Display clients/applicants last -->
                                <?php if (!empty($grouped_users['applicant'])): ?>
                                    <div class="contact-group">
                                        <h4 class="contact-group-title">Assigned Clients</h4>
                                        <?php foreach ($grouped_users['applicant'] as $client): ?>
                                            <div class="participant-option">
                                                <label>
                                                    <input type="radio" name="recipient_id" value="<?php echo $client['id']; ?>" required>
                                                    <div class="participant-info">
                                                        <div class="participant-avatar small">
                                                            <?php if (!empty($client['profile_picture'])): ?>
                                                                <img src="../../uploads/profiles/<?php echo $client['profile_picture']; ?>" alt="Profile">
                                                            <?php else: ?>
                                                                <div class="initials">
                                                        ...
                                                        <div class="participant-details">
                                                            ...
                                                            <span class="participant-role client-badge">
                                                                Client
                                                            </span>
                                                        </div>
                                                    </div>
                    ...
                    <div class="form-buttons">
                        ...
                        <button type="submit" name="create_direct_conversation" class="btn submit-btn" 
                                <?php echo empty($eligible_users) ? 'disabled' : ''; ?>>
                            Start Conversation
                        </button>
                    </div>
                </form>
