...
if (!$entity_id) {
    echo json_encode(['success' => false, 'error' => 'Professional profile not found']);
    exit;
}
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'delete_conversation':
                ...
                $stmt->bind_param("ii", $conversation_id, $entity_id);
                if ($stmt->execute()) {
            ...
            case 'create_conversation':
                ...
                $sql = "INSERT INTO ai_chat_conversations (entity_id, title, chat_type) 
                        VALUES (?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("iss", $entity_id, $title, $chat_type);
                if ($stmt->execute()) {
                    ...
                    $sql = "INSERT INTO ai_chat_messages (conversation_id, entity_id, role, content) 
                            VALUES (?, ?, 'assistant', ?)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("iis", $conversation_id, $entity_id, $welcome_msg);
                    $stmt->execute();
            ...
            case 'get_conversation':
                ...
                $sql = "SELECT id, role, content, created_at FROM ai_chat_messages 
                        WHERE conversation_id = ? AND entity_id = ? AND deleted_at IS NULL 
                ...
                if (!empty($message)) {
                    ...
                    try {
                        ...
                        $sql = "INSERT INTO ai_chat_messages (conversation_id, entity_id, role, content) 
                                VALUES (?, ?, 'user', ?)";
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param("iis", $conversation_id, $entity_id, $message);
                        if (!$stmt->execute()) {
                        ...
                        $stmt->bind_param("sii", $short_title, $conversation_id, $entity_id);
                        $stmt->execute();
                        ...
                        // Update usage
                        $sql = "INSERT INTO ai_chat_usage (entity_id, month, message_count) 
                                VALUES (?, ?, 1)
